...
standalone Unix filter, but its live memory is `O(input bytes)` and it is not a
bounded streaming implementation. Words use only ASCII TAB/LF/VT/FF/CR/SPACE
