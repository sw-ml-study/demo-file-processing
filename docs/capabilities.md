...
| Standalone wc stdin filter | A compiled native artifact counts LF lines, ASCII-delimited words, and bytes for mixed, empty, and terminated pipes; host `wc` agrees and source-free `env -i` execution passes | Genuine Unix-filter execution is accepted, but `read_stdin()` and `tokenize_bytes` retain the complete input, so memory is `O(input bytes)` rather than streaming-bounded. |
