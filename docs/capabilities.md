...
| Compiled byte applications | With the repository passed as `--source-dir`, actual hexdump and histogram demos resolve includes and user functions, then fail on unsupported `If`; mechanically flattened controls reach the same boundary | Source loading and function lowering are present in the selected development binary. Neither application produces an artifact; control flow, Results, byte I/O, and broader array/text lowering precede useful parity testing. |
| Compiled format applications | With the repository passed as `--source-dir`, actual bounded WAV copy/invert and Ogg copy/rewrite demos resolve includes and user functions, then fail on unsupported `If`; dependency-concatenated controls reach the same boundary | Source loading and function lowering are present in the selected development binary. No native format artifact exists to compare or memory-audit; general application lowering blocks before RIFF/Ogg-specific parity is reached. |
