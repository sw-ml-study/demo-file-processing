...
| File modification time | Upstream source is implementing confined `file_metadata(path).modified_unix_ms`, but the selected debug and release interpreters do not expose it yet | Real date scanning, timestamp ordering, UTC rendering, unavailable-value handling, and platform parity remain blocked; the planned demo will use explicit bounded paths after the API is committed and rebuilt. |
...
| Application lowering | `args`, `arg`, `write_stdout`, whole/range `read_bytes`, `file_size`, append/write output, bit operations, functions, control flow, Results, and records lower in the adjacent development compiler. Exact probes show valid bytes followed by generated `ok(count)` text, while invalid bytes produce `err(...)`. `read_stdin`, `print`, `eprint`, and `exit` remain unsupported. | File/bit lowering and byte rejection have advanced, but pristine compiled binary stdout, process semantics, sink-error parity, and broader application operations still gate a useful CLI. |
...
| Interpreter media-app portfolio | Inspector, doctor, and WAV transformer pass a combined 104-test/31-suite default gate plus exact narrated and artifact oracles | Higher-level applications are accepted without weakening compiler, codec, binary-source, or measured-memory boundaries. |
