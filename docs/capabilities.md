...
| File-date indexing | Pure MLPL validates exact Unix milliseconds, returns a deterministic available/newest/UTF-8-path permutation, places unavailable rows last, converts negative and positive epochs to UTC components, and renders ISO-8601 milliseconds | Reusable sorting/formatting is accepted with synthetic metadata-shaped rows. Live `file_metadata` retrieval and cross-platform filesystem parity await a rebuilt interpreter supplied by sw-MLPL. |
...
| Standalone wc stdin filter | A compiled native artifact counts LF lines, ASCII-delimited words, and bytes for mixed, empty, and terminated pipes; host `wc` agrees and source-free `env -i` execution passes | Accepted only as a whole-input compiler milestone. `read_stdin()` and `tokenize_bytes` retain the complete input, so it does not satisfy the required streaming pipeline; memory is `O(input bytes)`. |
...
| Interpreter application portfolio | Inspector, doctor, WAV transformer, date index, wc, grep, and du pass a combined 120-test/35-suite default gate plus exact narrated, host, and artifact oracles | Higher-level applications are accepted without weakening compiler, codec, binary-source, traversal, or measured-memory boundaries. |
