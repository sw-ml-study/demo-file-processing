...
| Interpreter application portfolio | Inspector, doctor, WAV transformer, date index, wc, grep, du, and ls pass a combined 124-test/36-suite default gate plus exact narrated, host, and artifact oracles | Higher-level applications are accepted without weakening compiler, codec, binary-source, traversal, or measured-memory boundaries. |
